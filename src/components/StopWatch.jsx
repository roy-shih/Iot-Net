    ...
    <div key={id} className="flex flex-col items-center mt-5">
      ...
      { isStop ? (
        <div className={`grid grid-cols-${window.innerWidth < 765 ? 1 : 2} gap-2 mt-5`}>
          <button type="button" id="Del" className="bg-gray-500 hover:bg-gray-700  text-white py-2 text-2s opacity-0.9 rounded-full p-4 hover:drop-shadow-xl text-center" onClick={handleReset}>
            Del
          </button>
          <button type="button" id="Done" className="bg-green-500 hover:bg-green-700  text-white py-2 text-2s opacity-0.9 rounded-full p-4 hover:drop-shadow-xl text-center" onClick={handleDone}>
            Done
          </button>
        </div>
      ) : null}
